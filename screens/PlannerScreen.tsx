    ...
    <View style={{ backgroundColor: colors.bg, paddingTop: 20 }}>
      ...
      {!showGroceryList && (
        <View style={{  padding: "0 20px 14px", gap: 6 }}>
          {DAYS.map((d) => {
            ...
            return (
              <Pressable key={d} onPress={() => setActiveDay(d)} style={{ flex: 1, padding: 8, margin: 10, borderRadius: 12, borderWidth: 0, backgroundColor: isActive ? colors.primary : "#FFFFFF", position: "relative", cursor: "pointer" }}>
                ...
                {hasItems && !isActive && <Text style={{ position: "absolute", top: 10, right: 6, width: 5, height: 5, borderRadius: 3, backgroundColor: colors.accent }} />}
              </Pressable>
            );
          })}
        </View>
                ...
                {r ? (
                  <View style={{ flexDirection: "row", backgroundColor: "#FFFFFF", borderRadius: 14 }}>
                    <View style={{ width: 25, height: 68, backgroundColor: "#E8E0D5", flex: 1,  borderRadius: 14}}>
                      <Image src={r.image} alt={r.title} style={{ width: "100%", height: "100%", objectFit: "cover",  borderTopLeftRadius: 14, borderBottomLeftRadius: 14 }} />
                    </View>
                    <View style={{ paddingTop: 10, paddingBottom: 10, paddingLeft : 12, paddingRight: 12, flex: 1 }}>
                      ...
                      <Text style={{ marginTop: 3, fontSize: 13, color: colors.muted }}>{r.servings} servings · {r.prepTime + r.cookTime}min</Text>
                    </View>
                    <Pressable onPress={() => assign(activeDay, meal, null)} style={{ paddingLeft: 14, paddingRight: 14, backgroundColor: "none", borderWidth: 0, cursor: "pointer"}}>
      ...
      {picking && (
        <View style={{ position: "absolute", inset: 0, backgroundColor: "rgba(26,20,16,0.6)",  alignItems: "flex-end", zIndex: 50 }}>
          <ScrollView style={{ backgroundColor: colors.bg, borderRadius: 24, width: "100%", maxHeight: "70%", padding: 30 }}>
            ...
            {recipes.map((r) => (
              <Pressable key={r.id} onPress={() => assign(picking.day, picking.meal, r)} style={{  gap: 12, paddingTop: 10, paddingBottom: 10, borderBottomWidth: 1, borderBottomColor: colors.border, cursor: "pointer", alignItems: "center" }}>
