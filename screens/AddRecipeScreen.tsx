...
export default function AddRecipeScreen({ onAdd, onBack }: Props) {
  ...
  return (
    <View style={{ backgroundColor: colors.bg, minHeight: "100%" }}>
      {/* Header */}
      <View style={{ paddingTop: 16, paddingLeft: 20, paddingRight: 20, paddingBottom: 14, alignItems: "center", gap: 14 }}>
      ...
      {!mode && (
        <View style={{ paddingTop: 10, paddingBottom: 10, paddingLeft: 20, paddingRight: 20 }}>
          <Text style={{ fontSize: 14, color: colors.textSoft, marginBottom: 20, lineHeight: 1.6 }}>
      ...
      {mode === "url" && !parseFeedback && (
        <View style={{ padding: "0 20px" }}>
          <Text style={{ fontSize: 14, color: colors.textSoft, marginBottom: 16, lineHeight: 1.6 }}>
          ...
          <Pressable
            ...
            style={{ width: "100%", backgroundColor: parsing ? colors.muted : colors.primary, borderWidth: 0, borderRadius: 14, paddingTop: 14, paddingBottom: 14, paddingLeft: 14, paddingRight: 14, fontSize: 16, fontWeight: 600, cursor: "pointer", fontFamily: "'Outfit', sans-serif", opacity: !url ? 0.5 : 1, transition: "all 0.2s" }}
          >
            {parsing ? "Parsing recipe…" : "Parse Recipe"}
          </Pressable>
          {parsing && (
        ...
        <View style={{ padding: "0 20px 24px" }}>
          ...
          {step === 1 && (
            <>
              ...
              <View style={{ marginBottom: 14 }}>
                <Text style={{ margin: "0 0 6px", fontSize: 12, color: colors.muted, fontWeight: 500, letterSpacing: 0.3 }}>FOLDER</Text>
                <View style={{ flexWrap: "wrap", gap: 7 }}>
                  {FOLDERS.map((f) => (
                    <Pressable key={f} onPress={() => setFolder(f)} style={{ padding: "6px 12px", borderRadius: 12, borderWidth: folder === f ? 0 : 1, borderColor: folder === f ? "transparent" : colors.border, backgroundColor: folder === f ? colors.primary : "#FFFFFF", cursor: "pointer" }}>
                      <Text style={{color: folder === f ? "#FAF7F2" : colors.textSoft, fontSize: 12, fontFamily: "'Outfit', sans-serif" }}>{f}</Text>
                    </Pressable>
                  ))}
                </View>
              </View>
          ...
          {step === 2 && (
            <>
              <Text style={{ margin: "0 0 6px", fontSize: 13, fontWeight: 600, color: "#1A1410" }}>Ingredients</Text>
              <Text style={{ margin: "0 0 12px", fontSize: 12, color: colors.muted }}>One per line, e.g. "200 g butter"</Text>
              <TextInput
                multiline={true}
                ...
                rows={10}
                style={{ width: "100%", backgroundColor: "#FFFFFF", borderColor: colors.border, borderWidth: 1, borderRadius: 12, paddingTop: 12, paddingBottom: 12, paddingLeft: 14, paddingRight: 14, fontSize: 14, color: "#1A1410", fontFamily: "'Outfit', sans-serif", boxSizing: "border-box", lineHeight: 1.7 }}
              />
            </>
          )}
          ...
          {step === 3 && (
            <>
              ...
              <TextInput
                ...
                rows={12}
                style={{ width: "100%", backgroundColor: "#FFFFFF", borderWidth: 1, borderColor:colors.border, borderRadius: 12, paddingTop: 12, paddingBottom: 12, paddingLeft: 14, paddingRight: 14, fontSize: 14, color: "#1A1410", fontFamily: "'Outfit', sans-serif", boxSizing: "border-box", lineHeight: 1.7 }}
              />
            </>
          )}
          ...
          <View style={{ gap: 10, marginTop: 20 }}>
            {step > 1 && (
              <Pressable onPress={() => setStep(step - 1)} style={{ flex: 1, backgroundColor: colors.primary, borderWidth: 0, borderRadius: 14, padding: "14px", cursor: "pointer" }}>
                <Text style={{color: colors.primary, fontSize: 15, fontWeight: 600, fontFamily: "'Outfit', sans-serif"}}>Back</Text>
              </Pressable>
            )}
            {step < 3 ? (
          <Pressable onPress={() => setStep(step + 1)} style={{ flex: 2, backgroundColor: colors.primary, borderWidth: 0, padding: "14px", cursor: "pointer" }}>
                <Text style={{color: "#FAF7F2", fontSize: 15, fontWeight: 600, cursor: "pointer", fontFamily: "'Outfit', sans-serif" }}>Continue</Text>
              </Pressable>
            ) : (
              <Pressable onPress={submit} style={{ flex: 2, backgroundColor: colors.accent, borderWidth: 0, borderRadius: 14, padding: "14px", cursor: "pointer" }}>
